...
with tab1:
    ...
    if run_btn:
        ...
        try:
            ...
            st.session_state["_result_mode"] = st.session_state.get("strategy_mode", "breakout")
    ...
    if "result" not in st.session_state:
        ...
        # ===== 切換策略模式 → 自動用快取即時重算（免重抓）=====
        _cur_mode = st.session_state.get("strategy_mode", "breakout")
        if (st.session_state.get("scan_cache")
                and st.session_state.get("_result_mode") not in (None, _cur_mode)):
            cfg = build_cfg()
            try:
                with st.spinner(f"切換為「{_mode_opts.get(_cur_mode, _cur_mode)}」，即時重算（免重抓）…"):
                    new_result = run_scan(cfg=cfg, cached=st.session_state["scan_cache"])
                new_result["scan_cache"] = st.session_state["scan_cache"]
                st.session_state["result"] = new_result
                st.session_state["cfg"] = cfg
                st.session_state["_result_mode"] = _cur_mode
                st.rerun()
            except Exception as e:
                st.session_state["_result_mode"] = _cur_mode   # 避免反覆重試
                st.error(f"即時切換失敗：{e}")
        ...
        if st.session_state.get("scan_cache"):
            ...
            rc1.caption(f"目前資料：{st.session_state.get('last_scan_at', '')} 抓取。"
                        f"**策略模式切換已即時生效**；換「風格 Preset / 權重」後按右鈕重評估。")
...
with tab5:
    ...
    st.markdown("---")
    st.markdown("### 📊 進階：怎麼讀表格、怎麼分析持股")

    with st.expander("Ⓐ 各模式「該看哪個指標」（重要）", expanded=False):
        st.markdown(
            "不同策略模式，看的主指標不一樣，別看錯：\n\n"
            "| 模式 | **主看** | RS 角色 |\n"
            "|---|---|---|\n"
    ...
    with st.expander("Ⓑ RS 相對強度怎麼看", expanded=False):
        st.markdown(
            "**RS% = 個股 60 日報酬 − 大盤 60 日報酬**，衡量「比大盤強還弱」。\n\n"
            "- **RS > 0**：贏大盤＝領漲股，啟動成功率/續航力較高 ← 突破模式優先\n"
            "- **RS ≈ 0**：跟大盤同步\n"
            "- **RS 負很多（如 −30）**：落後股，補漲性質 → **到目標就走、別當主流飆股抱**\n\n"
            "RS 負不代表不能買（潛伏/補漲就是賭這個），但要知道「它本來是弱的」。"
        )

    with st.expander("Ⓒ 怎麼自己分析一檔持股（買進後）", expanded=False):
        st.markdown(
            "拿到一檔持股，依序檢查（也可丟「持股管理」分頁自動跑）：\n\n"
            "**1. 算損益與 R 倍數**\n"
            "- 損益% =（現價 − 你的成本）/ 成本\n"
            "- R 倍數 =（現價 − 成本）/（成本 − 停損）。**達 +1R 賣半、+2R 出清**\n\n"
            "**2. 看位置（買貴了沒）**\n"
            "- 收盤乖離 MA20 太多（如 >8%）＝買在追高尾段，停損要收緊\n"
            "- 距 20 日高：負(已突破)、正(還沒突破)\n\n"
            "**3. 看籌碼動向（撐不撐得住）**\n"
            "- 法人連買天數、外資是否同步買 → 持續吸籌＝有撐；轉賣超＝警訊\n\n"
            "**4. 設好停損與目標**\n"
            "- 停損：系統停損價，或更緊的 進場−1.5×ATR\n"
            "- 目標：目標價1(先賣半) / 目標價2(出清)\n\n"
            "**5. 看 RS 決定抱多久**\n"
            "- RS 弱(落後股)：到目標就走，別期待大波段\n"
            "- RS 強(領漲股)：可用移動停利(ATR)續抱波段\n\n"
            "**範例**：買 34.3、現 35.85、停損 33.55 → R =（35.85−34.3）/（34.3−33.55）≈ **+2R**，"
            "已達標 → 先賣半鎖利、停損移到成本價保本。"
        )
